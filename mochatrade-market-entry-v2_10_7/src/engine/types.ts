...
/**
 * Round 2 research notes (data/round2-research.json, imported from the team's
 * research workbook). Text keeps the workbook's evidence tags: [V] verified,
 * [V-secondary], [V-draft], [A] assumption / limited evidence, [D] derived.
 */
export interface ResearchNotes {
  adaptation_cost?: number; // researcher-derived 1-5 [D]; NOT used for sequencing
  adaptation_rationale?: string;
  execution_dependency?: number; // researcher-derived 1-5 [D]; NOT used for sequencing
  ...
  sources?: string[];
  flags?: string; // open questions the researcher flagged
  deck_check?: string; // Round 1 markets only: where the research refines the deck
  reserve_reason?: string; // screened-out reserve markets only
}
...
export interface Market {
  ...
  /** Research notes: on the six Round 1 screened-out markets (alongside the deck text) and on every Round 2 market. */
  research?: ResearchNotes;
  /** "round2": a market added from the Round 2 research workbook, not the Round 1 deck. Scored and ranked, never sequenced (no entry route, timing or go-gates yet). */
  research_source?: "round2";
  /** Which workbook list a Round 2 market came from. */
  research_list?: "additional" | "reserve";
  ...
  iso3?: string; // ISO-3166 alpha-3, used only for the flag on the market page
  why_not_now?: WhyNotNow; // Do-not-enter markets only: rubric-by-rubric reason, from data/why-not-now.json
...
/** Plain-language "why not now" for a Do-not-enter market, written strictly against the rubric dimensions (data/why-not-now.json). */
export interface WhyNotNow {
  summary: string;
  dimensions: Array<{ key: DimensionKey; note: string }>;
...
export interface Decision {
  ...
  window: string | null;
  conditions: string[]; // go-gates + route confirmations that must close before launch; for a no-go with why_not_now, one line per failing rubric dimension
  whyNotNow?: WhyNotNow | null;
  risk: RegulatoryRisk;
