...
import { go, openMarket, openTab, queueOrder, renderApp } from "./harness";
...
describe("Market-selection persistence edge case", () => {
  it("viewing Brazil, then changing weights, keeps Brazil selected and only its DATA updates -- the app must not silently jump the user to a different market", async () => {
    ...
    await openMarket(user, "brazil", "scores");
    expect(screen.getByTestId("scorecard").dataset.marketId).toBe("brazil");

    // Switch to a preset that reshuffles the #1 sequenced market (on "How we decided"), then come back.
    go("#/method");
    await user.click(screen.getByTestId("preset-capital_tight"));
    go("#/market/brazil/scores");

    // Still Brazil -- the market is in the URL, and a weight change never redirects.
    expect(window.location.hash).toBe("#/market/brazil/scores");
    expect(screen.getByTestId("scorecard").dataset.marketId).toBe("brazil");
  ...
  it("switching weights while a screened-out market is selected does not bounce the user to a cleared market", async () => {
    ...
    await openMarket(user, "thailand", "scores");
...
describe("Recommendation-detail correctness across market + weight changes", () => {
  it.each(engine.getAllMarkets().map((m) => m.id))(
    ...
    async (marketId) => {
      ...
      // Blockers appear once on the decision tab: either as a "Before we
      // launch" condition on the decision card, or under "What could block
      // us" -- never both (v2.10.2 dedupe). Check the tab, and check that the
      // summary does not repeat a condition the card already lists.
      const decisionTab = screen.getByTestId("decision-card").parentElement!.textContent ?? "";
      const conditions = screen.getByTestId("decision-card").textContent ?? "";
      rec.blockers.forEach((b) => {
        expect(decisionTab).toContain(b.text);
        if (conditions.includes(b.text)) expect(recSummary.textContent).not.toContain(b.text);
      });
...
describe("No stale state left behind when rapidly switching market and weights together", () => {
  it("market -> preset -> market -> preset in sequence always ends in a consistent state with no leftover previous-market content", async () => {
    ...
    go("#/method");
    await user.click(screen.getByTestId("preset-regulation_heavy"));
    await openMarket(user, "south_africa");
    go("#/method");
    await user.click(screen.getByTestId("preset-market_heavy"));
    go("#/market/south_africa/scores");
    ...
    expect(screen.getByTestId("compliance-checklist").dataset.mode).toBe("screened-out");
    await openTab(user, "risks");
    // South Africa now has risks tagged (no_licensed_route, partner_single_point, derivatives_product_gap).
...
describe("Round 2 additions: decision card, compare, add market", () => {
  it("shows a verdict and a regulatory risk band for the selected market", async () => {
    ...
    const user = userEvent.setup();
    renderApp("#/plan");
    // Was: the default selection. Now: the plan's #1, opened from the plan page's primary button.
  ...
  it("a user-entered market is ranked and sequenced with the others", async () => {
    ...
    // Adding a market opens its page.
    expect(window.location.hash).toBe("#/market/custom_kenya");
