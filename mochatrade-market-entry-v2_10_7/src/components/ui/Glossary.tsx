...
export default function Gloss({ text, className }: { text: string; className?: string }): ReactElement {
  ...
  return (
    <span className={className}>
      {parts.map((p, i) => (typeof p === "string" ? p : <Term key={i} term={p.term} />))}
    </span>
  );
}

/**
 * One glossary term. Hover, focus or tap shows a definition card below the
 * term (above it when there is no room), in the site's own type and colours
 * instead of the browser's plain tooltip. The card is rendered through a
 * portal so no ancestor's overflow clips it; it closes on mouse-out, blur,
 * Escape, scroll, or a tap elsewhere.
 */
function Term({ term }: { term: string }): ReactElement {
  ...
  return (
    <span
      ...
      onClick={() => setOpen((o) => !o)}
      className="cursor-help rounded-[3px] underline decoration-muted decoration-dotted underline-offset-[3px] transition-colors hover:bg-surface-2 hover:decoration-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ink"
    >
      {term}
      {open && pos && definition &&
        createPortal(
          <span
            id={id}
            role="tooltip"
            data-testid="gloss-card"
            className="pop-in fixed z-50 block w-[min(340px,calc(100vw-24px))] rounded-[14px] border border-rule bg-surface p-4 text-left shadow-float"
            style={{ left: pos.left, top: pos.top, transform: pos.above ? "translateY(-100%)" : undefined }}
          >
            <span className="block font-sans text-[15px] font-bold leading-snug text-ink">{term}</span>
            <span className="mt-1.5 block font-sans text-[15px] font-normal leading-relaxed text-ink-2">{definition}</span>
          </span>,
          document.body,
        )}
    </span>
