    ...
    <div data-testid="ranking-vs-sequence">
      <div className="mb-4 flex items-center justify-between">
        <div className="text-[14px] text-muted">
          <span className="text-ink-2">By screening score</span>
        ...
        <div className="text-right text-[14px] text-muted">
          ...
          <span className="text-ink-2">Recommended entry order</span>
        </div>
      </div>
      ...
      <div className="relative" style={{ height }}>
        ...
        {byRaw.map((it, i) => (
          ...
          >
            <span
              className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full border text-[14px] tabular"
              style={{ borderColor: colorOf(it.sequenced.market.id), color: colorOf(it.sequenced.market.id) }}
            >
              {it.divergence.rawRankAmongCleared}
            </span>
            <div>
              <div className="font-display text-[17px] text-ink">{it.sequenced.market.name}</div>
              <div className="text-[14px] text-muted">{formatScore(it.sequenced.screening.weightedScore)} / 5</div>
            </div>
          </div>
        ))}
        ...
        {bySeq.map((it, i) => (
          ...
          >
            <div>
              <div className="font-display text-[17px] text-ink">{it.sequenced.market.name}</div>
              <div className="text-[14px] text-muted">{it.sequenced.market.sequence?.window}</div>
            </div>
            <span
              className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[14px] font-medium tabular"
      ...
      {items.length > 1 && lam.holds && (
        <p data-testid="lambda-robustness" className="mt-4 text-[15px] leading-snug text-muted">
      ...
      {anyDiverges && (
        <div className="mt-5 space-y-2 border-t border-rule pt-4">
          {items
            ...
            .map((it) => (
              <p key={it.sequenced.market.id} className="max-w-[72ch] text-[17px] leading-relaxed text-ink-2"><span className="mr-2 inline-block h-2.5 w-2.5 rounded-full align-middle" style={{ backgroundColor: colorOf(it.sequenced.market.id) }} aria-hidden="true" />
...
function FrictionBars({ items }: { items: RankVsSeqItem[] }): ReactElement {
  ...
  return (
    <div data-testid="friction-bars" className="mt-6 border-t border-rule pt-4">
      <div className="mb-3 flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
        <p className="text-[17px] text-ink">Score, minus the cost of getting in</p>
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[15px] text-ink-2">
          <span className="flex items-center gap-1.5"><span className="h-2.5 w-4 rounded-[2px] bg-ink-2" />what is left decides the order</span>
          ...
          return (
            <div key={s.market.id} data-testid={`friction-${s.market.id}`} className="grid grid-cols-[minmax(92px,120px)_1fr] items-center gap-3" title={title}>
              <div className="flex items-center gap-2">
                <span className="text-[15px] tabular" style={{ color }}>#{s.sequenceRank}</span>
                <span className="truncate text-[17px] text-ink">{s.market.name}</span>
              </div>
              <div className="relative h-4 rounded-[3px] bg-surface-2" role="img" aria-label={title}>
                ...
                <div className="absolute inset-y-[-3px] w-[2px] bg-ink" style={{ left: pct(score) }} />
              </div>
            </div>
          );
        })}
      ...
      <p className="mt-2 text-[15px] leading-snug text-muted">The dark tick is the screening score. Hover a bar for the exact figures.</p>
    </div>
  );
}
