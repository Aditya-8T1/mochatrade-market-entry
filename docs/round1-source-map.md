...
2. **`adaptation_cost` and `execution_dependency` scores (1-5)** for Dubai/Brazil/Indonesia. The deck argues the sequence qualitatively (Dubai = lowest capital + closest fit; Brazil = worth a product rebuild; Indonesia = highest partner + localisation burden) but never assigns numbers. These were reverse-engineered specifically so the sequencing engine's formula reproduces Dubai -> Brazil -> Indonesia. This is the single most important thing to sanity-check as a team before the demo -- if a judge asks "how exactly did you turn 'highest execution dependency' into a number," the honest answer right now is "the team estimated it for the prototype," and that's fine to say, but say it, don't imply it was in the original scoring.
